...
public class MultiBlockGameObjectScript : MonoBehaviour
{
    ...
    public void OnMouseDrag()
    {
        if (uiController.EditMode)
        {
            ...
            Vector3 pos = new Vector3((float) Math.Round(world_point_mousePos.x) + (structure.getDimensions().x - 1) / 2, (float) Math.Round(world_point_mousePos.y) + (structure.getDimensions().y - 1) / 2);
